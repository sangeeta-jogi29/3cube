    ...
    <div className="bg-white text-gray-900 font-sans">
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-8 md:gap-20">
          <div className="max-w-xl flex flex-col gap-4">
            <h1 className="text-4xl md:text-5xl font-extrabold">
              <span className="bg-white text-primary-700 px-2 py-1 rounded">Contact</span> Us
            </h1>
            ...
            <nav className="mt-4 text-sm font-medium opacity-70" aria-label="Breadcrumb">
              <ol className="flex items-center space-x-1 text-primary-200">
          ...
          <div className="flex-grow h-48 md:h-64 bg-primary-500 rounded-lg opacity-25" aria-hidden="true"></div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 mt-20 flex flex-col md:flex-row items-start gap-12 md:gap-24">
        <div className="flex-1 max-w-md space-y-6">
          <p className="uppercase text-sm font-semibold tracking-wide text-primary-700">Lets Connected</p>
        ...
        <div className="flex-1 max-w-md bg-primary-100 rounded-2xl p-8 relative">
          <blockquote className="relative text-gray-800 text-base">
            <span className="material-icons absolute top-6 left-6 text-primary-600 text-4xl select-none">format_quote</span>
            <p className="pl-12">
              Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ut elit tellus, luctus nec ullamcorper mattis.
            </p>
          </blockquote>
          <footer className="mt-6 font-semibold">
            Mrs. John Kole
            <span className="block text-sm text-primary-600 tracking-widest uppercase">Head Director</span>
          </footer>
          <img
          ...
          <form onSubmit={handleContactSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" value={formData.firstName} onChange={e => setFormData({ ...formData, firstName: e.target.value })} required className="w-full rounded-md border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-primary-600" />
              </div>
              <div>
                <label className="block mb-1 font-medium text-gray-700">Last Name</label>
                <input type="text" value={formData.lastName} onChange={e => setFormData({ ...formData, lastName: e.target.value })} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-primary-600" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="email" value={formData.email} onChange={e => setFormData({ ...formData, email: e.target.value })} required className="w-full rounded-md border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-primary-600" />
              </div>
              <div>
                <label className="block mb-1 font-medium text-gray-700">Subject</label>
                <select value={formData.subject} onChange={e => setFormData({ ...formData, subject: e.target.value })} className="w-full rounded-md border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-primary-600">
            ...
            <div>
              ...
              <textarea value={formData.message} onChange={e => setFormData({ ...formData, message: e.target.value })} rows="4" required className="w-full rounded-md border border-gray-300 px-4 py-2 resize-y focus:ring-2 focus:ring-primary-600"></textarea>
            </div>

            <button type="submit" className="w-full bg-primary-600 text-white py-3 rounded-md hover:bg-primary-700 transition font-semibold">Send Message</button>
          </form>
        </div>
      </section>
      ...
      <section className="bg-gray-50 mt-24 py-8 px-6 md:px-16">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <form onSubmit={handleNewsletterSubmit} className="flex w-full max-w-md gap-2">
            <input type="email" placeholder="Email Address" value={newsletterEmail} onChange={e => setNewsletterEmail(e.target.value)} required className="flex-grow rounded-md border border-gray-300 px-4 py-2 focus:ring-2 focus:ring-primary-600" />
            <button type="submit" className="bg-primary-600 text-white rounded-md px-6 py-2 font-semibold hover:bg-primary-700 transition">Subscribe</button>
          </form>
        </div>
      </section>
    </div>
